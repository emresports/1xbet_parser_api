...
var force_data = function(data, sport_id, pkg_data, lang){
	var sport_no_block = [];
	...
	for(var i=0; i<data.length; i++){
		...
		if(sport_id!=0 && sport_id!=data[i]["id"]){
			continue;
		}
		...
		for(var j=0; j<data[i]["sub"].length; j++){
			...
			delete data[i]["sub"][j]["sub"];

			if(typeof data[i]["sub"][j]["name_"+lang]!="undefined"){
				data[i]["sub"][j] = data[i]["sub"][j];
			}

			ret_data.push(data[i]["sub"][j]);
		}
	}
...
module.exports = async function(sport_id, type, lang, pkg, cb){
	...
	var main_cache_filename = app_config.root_dir+""+app_config.collector_dir+"/cache_"+type.toLowerCase()+"_"+lang+"_countries"+sport_file+"_main_storage.json";
	...
	let forced_data = force_data(data, sport_id, rows[0], lang);
